...
"""
olfactory_category_umap.py

Script that runs Uniform Manifold Approximation and Projection (nonlinear dimensionality reduction method) on a food category in 
the VCF dataset. 

Usage:
    python3 olfactory_category_umap.py
...
def main(config_path) -> None: 
    try:
        ...
        plt.title(f"UMAP with {umap_components} Components on Food Category: {category}", fontsize=8)
        plt.scatter(embedding[:, 0], embedding[:, 1], c=combined_colors, s=5)
        plt.legend(handles=patches, fontsize=8)
        if store_results:
            plt.savefig(f"{output_folder}/umap_{category}_2d.pdf")
        plt.show()
