...
sampleword_corr_matrix = np.load('./sampleword_corr_matrix.npy')
smellword_corr_matrix = np.load('./smellword_corr_matrix.npy')
vcf_corr_matrix = np.load('./vcf_binary_corr_matrix.npy')

# ===== OBTAINING THE UPPER TRIANGLE OF CORRELATION MATRIX AND CORRELATION STATS =====
text_rep_vals = []
vcf_rep_vals = []
for x in range(0, sampleword_corr_matrix.shape[0]):
    for y in range(0, x):
        text_rep_vals.append(sampleword_corr_matrix[x][y])
        vcf_rep_vals.append(vcf_corr_matrix[x][y])

print(len(text_rep_vals))
...
ax.set_ylabel('VCF Binary Representation')
...
if store_results:
    plt.savefig(f"{output_folder}/sampleword_vcfbinary_representation_similarity_plot.pdf")
plt.show()